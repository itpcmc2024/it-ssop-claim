/*
======================================================
PCMC-SSO Toolkit Professional Edition V4.5.9
...
function syncRuntimeVersionLabels_(){
 const v=String(window.SSOP_CONFIG?.version||'').trim(); if(!v)return;
 document.querySelectorAll('.version').forEach(el=>{el.textContent='V'+v;});
 document.querySelectorAll('.copyright-footer strong').forEach(el=>{el.textContent=String(el.textContent||'').replace(/Version\s+\d+\.\d+\.\d+/g,'Version '+v);});
 const about=document.querySelector('#aboutModal p'); if(about)about.innerHTML=String(about.innerHTML||'').replace(/Version\s+\d+\.\d+\.\d+/g,'Version '+v);
}
document.addEventListener('DOMContentLoaded',syncRuntimeVersionLabels_);
...
const REGISTRY_MODULE_CONFIG={
 SSOCAC:{title:'ทะเบียนงาน Cancer Care',subtitle:'ทะเบียนผู้ป่วยหลัง Discharge สำหรับเตรียมข้อมูลส่งเบิก SSOCAC',icon:'🎗️',footer:'PCMC-SSO Toolkit · Cancer Care Registry Version 4.5.9'},
 STCPAP:{title:'ทะเบียนงาน SSOCPAP',subtitle:'ทะเบียนผู้ป่วยสำหรับเตรียมข้อมูลส่งเบิกเครื่อง CPAP และหน้ากาก ตามกฎ STCPAP',icon:'🫁',footer:'PCMC-SSO Toolkit · SSOCPAP Registry Version 4.5.9'},
 STSLEEP:{title:'ทะเบียนงาน Sleep Test',subtitle:'ทะเบียนผู้ป่วยสำหรับเตรียมข้อมูลส่งเบิก Sleep Test ตามกฎ STCPAP',icon:'🌙',footer:'PCMC-SSO Toolkit · Sleep Test Registry Version 4.5.9'}
};
...
const CROSS_RULE_GUIDE={
 ...
 R31:{level:'Error',title:'มีการเบิกค่ายาแต่ไม่มี BILLDISP',solution:'ตามคู่มือ SSOP: ถ้า BillItems หมวด 3/5 มีการขอเบิก ต้องมี BILLDISP (Dispensing + DispensedItems) ประกอบเสมอ. วิธีแก้มี 2 ทางตามข้อเท็จจริง: (A) เป็นยาที่ขอเบิกจริง → เติม BILLDISP จากข้อมูลจ่ายยาจริง; (B) เป็นรายการที่ผู้ป่วยชำระเองและไม่ควรขอเบิก → ตัดยอดขอเบิกของรายการยาออก แล้วตรวจสมการ BILLTRAN.Amount = Paid + ClaimAmt + OtherPay และ T45/T44 ต่อ. ระบบห้ามสร้างข้อมูลยาโดยเดา'},
 ...
 T32:{level:'Error',title:'Paid + ClaimAmt + OtherPay ไม่เท่ากับ Amount',solution:'ตรวจ BILLTRAN ตามสมการ Amount = Paid + ClaimAmt + OtherPay. ใช้ตรวจยอดหลังแก้ R31/T45 และตรวจว่าตัวเลขไม่มี comma หลักพัน'},
...
function scanCrossZip_(){
 ...
 bill.forEach(o=>{
  ...
  const amount32=crossNum_(o.row[8]),paid32=crossNum_(o.row[9])||0,claim32=crossNum_(o.row[16])||0,other32=crossNum_(o.row[18])||0;
  if(amount32!==null&&Math.abs(amount32-(paid32+claim32+other32))>0.01)crossAddIssue_(issues,'T32',o,`InvNo ${inv}: Amount ${amount32.toFixed(2)} ≠ Paid ${paid32.toFixed(2)} + ClaimAmt ${claim32.toFixed(2)} + OtherPay ${other32.toFixed(2)}`,inv);
...
const CROSS_AUTO_FIX_LOGIC={
 ...
 R31:{title:'ปรับรายการยาเป็นผู้ป่วยชำระเอง (เฉพาะกรณียอดตรงกับ Paid)',steps:['ตรวจ InvNo ที่ติด R31 และรวม BillItems.ClaimAmount เฉพาะหมวด 3/5','อนุญาต Auto Repair เฉพาะเมื่อยอดขอเบิกยารวม = BILLTRAN.Paid และ Paid > 0','ตั้ง BillItems.ClaimUP และ ClaimAmount ของหมวด 3/5 เป็น 0 เพื่อไม่ให้ T44 ตามมา','คำนวณ BILLTRAN.ClaimAmt ใหม่จากผลรวม BillItems.ClaimAmount ทั้ง InvNo','ตรวจสมการ Amount = Paid + ClaimAmt + OtherPay และตรวจ T32/T44/T45 ใหม่'],fields:'BillItems.ClaimUP / ClaimAmount → 0; BILLTRAN.ClaimAmt → SUM(BillItems.ClaimAmount)',safety:'นี่เป็น Guided Repair สำหรับกรณีที่ยืนยันแล้วว่าเป็นยอดผู้ป่วยชำระเองเท่านั้น. ถ้า Paid ไม่เท่ากับยอดขอเบิกยารวม ระบบจะไม่แก้อัตโนมัติ และต้องเติม BILLDISP หรือแก้ตามข้อมูลจริง'},
};
...
const CROSS_BULK_FIXABLE=new Set(['C08','S32','S33','R04','R31','T33','T44','T45']);
...
async function bulkFixSelectedCrossCode_(){const code=document.getElementById('crossIssueCodeFilter')?.value||'ALL';if(!CROSS_BULK_FIXABLE.has(code)){toast('ยังไม่รองรับการแก้อัตโนมัติทั้งหมด','เลือก Error Code ที่ระบบคำนวณค่าแก้ไขได้อย่างปลอดภัย เช่น C08, S32, S33, R04, T33, T44 หรือ T45','warning',6000);return;}let issues=(zipReaderState.crossIssues||[]).filter(x=>x.code===code&&x.entry);if(!issues.length)return;
 if(code==='R31'){const ix0=crossSectionIndex_();issues=issues.filter(issue=>{const o=findZipOrigin_(issue);if(!o?.row)return false;const inv=String(o.row[4]||'').trim(),paid=crossNum_(o.row[9])||0,drug=(ix0.biByInv.get(inv)||[]).filter(x=>['3','5'].includes(String(x.row[2]||'').replace(/^0+/,''))).reduce((a,x)=>a+(crossNum_(x.row[10])||0),0);return paid>0&&Math.abs(drug-paid)<=0.01;});if(!issues.length){toast('R31 ยังแก้อัตโนมัติไม่ได้','Auto Repair ใช้ได้เฉพาะ InvNo ที่ยอดขอเบิกยาหมวด 3/5 เท่ากับ BILLTRAN.Paid และยืนยันว่าเป็นยอดผู้ป่วยชำระเอง. กรณีอื่นต้องเติม BILLDISP หรือแก้ตามข้อมูลจริง','warning',9000);return;}}
 const repairLabel=code==='R31'?'Guided Repair R31':'Auto Fix '+code;const ok=await showDialog(`${repairLabel} · Logic วิธีแก้`,crossAutoFixExplanation_(code,issues.length),'warning',[{text:'ยกเลิก',value:false,className:'soft'},{text:code==='R31'?'ยืนยัน Guided Repair R31':`ยืนยัน Auto Fix ${code}`,value:true,className:'primary'}]);if(!ok)return;const ix=crossSectionIndex_();let changed=0;
 for(const issue of issues){const {item,row}=findZipOrigin_(issue);if(!item||!row)continue;const inv=String(issue.key||'').trim(),bill=(ix.billByInv.get(inv)||[])[0]?.row;
  ...
  else if(code==='R31'&&issue.section==='BILLTRAN'){
    const inv31=String(row[4]||'').trim(),paid31=crossNum_(row[9])||0,items31=ix.biByInv.get(inv31)||[];
    const drugItems31=items31.filter(x=>['3','5'].includes(String(x.row[2]||'').replace(/^0+/,'').trim()));
    const drugClaim31=drugItems31.reduce((a,x)=>a+(crossNum_(x.row[10])||0),0);
    if(paid31<=0||Math.abs(drugClaim31-paid31)>0.01)continue;
    drugItems31.forEach(x=>{const targetItem=zipReaderState.entries.find(e=>e.name===x.entry);if(!targetItem)return;if(setCrossBulkCell_(targetItem,'BillItems',x.row,9,'0.00'))changed++;if(setCrossBulkCell_(targetItem,'BillItems',x.row,10,'0.00'))changed++;});
    const newClaim31=items31.reduce((a,x)=>a+(crossNum_(x.row[10])||0),0);
    if(setCrossBulkCell_(item,'BILLTRAN',row,16,newClaim31.toFixed(2)))changed++;
   }
...
const CROSS_KNOWLEDGE_SEED_V459=[
 ...
 {ErrorCode:'R31',Description:'ใน BillTran มีการเบิกค่ายา แต่ขาดข้อมูลยาใน BillDisp',Cause:'ตามคู่มือ SSOP เมื่อ BillItems.BillMuad หมวด 3 หรือ 5 มีการขอเบิก จะต้องส่ง BILLDISP ประกอบเสมอ',Solution:'ตรวจตาม InvNo: ถ้าเป็นยาที่ขอเบิกจริง ให้เติม Dispensing และ DispensedItems จากข้อมูลจ่ายยาจริง. ถ้าเป็นยอดที่ผู้ป่วยชำระเองและไม่ควรขอเบิก ให้ตัด ClaimAmount ของรายการยานั้นออก ปรับ ClaimAmt ให้เท่าผลรวม BillItems และตรวจ T32/T44/T45 ต่อ',RelatedFile:'BILLTRAN / BillItems / BILLDISP',RelatedField:'InvNo / BillMuad / Paid / ClaimAmt / ClaimUP / ClaimAmount',Tips:'Built-in Guided Repair ใช้ได้เฉพาะเมื่อยอดขอเบิกยาหมวด 3/5 เท่ากับ BILLTRAN.Paid; ระบบจะตั้ง ClaimUP/ClaimAmount ของรายการยานั้นเป็น 0 และคำนวณ ClaimAmt ใหม่. ถ้ายอดไม่ตรง ห้ามแก้อัตโนมัติ'},
 ...
 {ErrorCode:'T32',Description:'Paid + ClaimAmt + OtherPay ไม่เท่ากับ Amount',Cause:'ยอดชำระเอง ยอดขอเบิก หรือยอดจากแหล่งอื่นรวมกันไม่เท่ากับยอดเรียกเก็บ',Solution:'ตรวจ BILLTRAN ตามสมการ Amount = Paid + ClaimAmt + OtherPay และตรวจตัวเลขไม่มี comma หลักพัน',RelatedFile:'BILLTRAN',RelatedField:'Amount / Paid / ClaimAmt / OtherPay',Tips:'Error; ใช้ตรวจหลังแก้ R31/T45'},
 {ErrorCode:'T43',Description:'BillItems.ChargeAmt ไม่เท่ากับ Quantity × UnitPrice',Cause:'ยอดเรียกเก็บรายการย่อยไม่สัมพันธ์กับจำนวนและราคาขายต่อหน่วย',Solution:'คำนวณ Quantity × UnitPrice แล้วตรวจ ChargeAmt; คู่มือยอมให้ผลคูณปัดเศษได้ไม่เกิน 1 บาท',RelatedFile:'BillItems',RelatedField:'Quantity / UnitPrice / ChargeAmt',Tips:'Error; ตรวจทีละรายการ'},
 {ErrorCode:'R02',Description:'จำนวน DispensedItems ไม่เท่ากับ Dispensing.Itemcnt',Cause:'จำนวนบรรทัดรายการยาจริงไม่เท่ากับ Itemcnt ที่หัว Dispensing',Solution:'นับ DispensedItems ตาม DispID แล้วแก้ Dispensing.Itemcnt หรือรายการย่อยให้ตรงข้อมูลจริง',RelatedFile:'BILLDISP',RelatedField:'DispID / Itemcnt',Tips:'Error'},
 {ErrorCode:'R03',Description:'DispID ใน DispensedItems เชื่อมกับ Dispensing ไม่ได้',Cause:'มี DispensedItems ที่ไม่พบ Dispensing.DispID แม่',Solution:'แก้ DispID ให้สัมพันธ์ หรือเติม Dispensing ที่ขาดจากข้อมูลจริง',RelatedFile:'BILLDISP',RelatedField:'DispID',Tips:'Error; ห้ามสร้างข้อมูลยาโดยเดา'},
 {ErrorCode:'R05',Description:'DispID ซ้ำในไฟล์เดียวกัน',Cause:'Dispensing มี DispID ซ้ำภายในชุดส่งเดียวกัน',Solution:'ตรวจรายการซ้ำและแก้ key/ลบรายการซ้ำตามข้อมูลจริง โดยต้องรักษาความสัมพันธ์ DispensedItems',RelatedFile:'BILLDISP',RelatedField:'DispID',Tips:'Error'},
 {ErrorCode:'R12',Description:'Reimburser ไม่ถูกต้อง',Cause:'ค่า Dispensing.Reimburser ไม่อยู่ใน codeset ที่ยอมรับ',Solution:'ตรวจและแก้ Reimburser ตาม codeset ปัจจุบันของ SSOP',RelatedFile:'BILLDISP',RelatedField:'Dispensing.Reimburser',Tips:'Error; ต้องยืนยัน codeset ปัจจุบัน'},
 {ErrorCode:'R22',Description:'ขาดข้อมูล PackSize',Cause:'รายการยา PrdCat 1-5 ไม่มีขนาดบรรจุ',Solution:'กรอก DispensedItems.PackSize สำหรับ PrdCat 1-5; PrdCat 6-7 ว่างได้ตามคู่มือ',RelatedFile:'BILLDISP',RelatedField:'PrdCat / PackSize',Tips:'Error'},
 {ErrorCode:'R23',Description:'ขาดชื่อ Dose / Form / Strength',Cause:'รายการยาไม่มีข้อมูล Dose/Form/Strength ที่จำเป็น',Solution:'ตรวจข้อมูลยาและเติม DispensedItems.Sigtext ให้ครบตามข้อมูลจริง',RelatedFile:'BILLDISP',RelatedField:'DispensedItems.Sigtext',Tips:'Error; ตรวจกับข้อมูลยาจริง'},
 {ErrorCode:'R24',Description:'ขาดข้อมูลวิธีการใช้ยา',Cause:'รายการยา PrdCat 1-5 ไม่มีวิธีใช้ยา',Solution:'กรอก DispensedItems.Sigtext สำหรับ PrdCat 1-5; PrdCat 6-7 ว่างได้ตามคู่มือ',RelatedFile:'BILLDISP',RelatedField:'PrdCat / Sigtext',Tips:'Error'},
 {ErrorCode:'R32',Description:'มีข้อมูลยาใน BillDisp แต่ไม่มีการเบิกค่ายาใน BillTran',Cause:'มีข้อมูล BILLDISP แต่ BillItems ไม่มีรายการหมวดยาที่สัมพันธ์',Solution:'ตรวจ InvNo ว่าควรมี BillItems หมวด 3/5 หรือ BILLDISP เป็นข้อมูลเกิน; แก้ตามข้อมูลจริง',RelatedFile:'BillItems / BILLDISP',RelatedField:'InvNo / BillMuad',Tips:'Error'},
 {ErrorCode:'R34',Description:'HN ในข้อมูลยาไม่ตรงกับ BILLTRAN',Cause:'HN ใน BillDisp และ BillTran ของธุรกรรมเดียวกันไม่ตรงกัน',Solution:'ใช้ InvNo เป็น key แล้วแก้ HN ให้เป็นผู้ป่วยรายเดียวกันทุกแฟ้ม',RelatedFile:'BILLTRAN / BILLDISP',RelatedField:'InvNo / HN',Tips:'Error'},
 {ErrorCode:'R41',Description:'ยอด Dispensing ไม่สมดุล',Cause:'Dispensing.ChargeAmt ไม่เท่ากับ ClaimAmt + Paid + Other',Solution:'ตรวจ Dispensing ตามสมการ ChargeAmt = ClaimAmt + Paid + Other และแก้ยอดตามข้อมูลจริง',RelatedFile:'BILLDISP',RelatedField:'ChargeAmt / ClaimAmt / Paid / Other',Tips:'Error'},
 {ErrorCode:'R42',Description:'DispensedItems.ChargeAmt ไม่เท่ากับ Quantity × UnitPrice',Cause:'ยอดราคาขายรายการยาไม่สัมพันธ์กับจำนวนและราคาต่อหน่วย',Solution:'คำนวณ Quantity × UnitPrice แล้วแก้ ChargeAmt/Quantity/UnitPrice ตามข้อมูลจริง; คู่มือยอมให้ปัดเศษได้ไม่เกิน 1 บาท',RelatedFile:'BILLDISP',RelatedField:'Quantity / UnitPrice / ChargeAmt',Tips:'Error'},
 {ErrorCode:'R43',Description:'DispensedItems.ReimbAmt ไม่เท่ากับ Quantity × ReimbPrice',Cause:'ยอดเบิกยาไม่สัมพันธ์กับจำนวนและราคาเบิกต่อหน่วย',Solution:'คำนวณ Quantity × ReimbPrice แล้วแก้ ReimbAmt/Quantity/ReimbPrice ตามข้อมูลจริง; คู่มือยอมให้ปัดเศษได้ไม่เกิน 1 บาท',RelatedFile:'BILLDISP',RelatedField:'Quantity / ReimbPrice / ReimbAmt',Tips:'Error'},
];
async function syncCrossKnowledgeSeed_(){if(String(authState.user?.Role||'').toUpperCase()!=='ADMIN'){toast('เฉพาะ Admin','การอัปเดต Knowledge มาตรฐานต้องใช้สิทธิ์ Admin','warning');return;}const ok=await showDialog('อัปเดต Cross Knowledge มาตรฐาน',`ระบบจะ Upsert ${CROSS_KNOWLEDGE_SEED_V459.length} Error/Warning โดยใช้ Module + ErrorCode เป็น key จึงไม่สร้างรายการซ้ำ และผู้ใช้ยังแก้/นำเข้า Error ใหม่ได้เหมือนเดิม`,'info',[{text:'ยกเลิก',value:false,className:'soft'},{text:'อัปเดต Knowledge',value:true,className:'primary'}]);if(!ok)return;try{const items=CROSS_KNOWLEDGE_SEED_V459.map(x=>({Module:'CROSS',...x,UpdatedBy:authState.user?.Display_Name||'V4.5.9 Knowledge Seed',Active:true}));const d=await apiRequest('upsertKnowledge',{items});cacheCrossKnowledgeItems_(items);toast('อัปเดต Knowledge สำเร็จ',`เพิ่ม ${d.inserted||0} · อัปเดต ${d.updated||0} · ไม่สร้างรายการซ้ำ`,'success',7000);await loadKnowledge(document.getElementById('knowledgeSearchInput')?.value||'',document.getElementById('knowledgeModuleFilter')?.value||'ALL');}catch(e){toast('อัปเดต Knowledge ไม่สำเร็จ',e.message,'error',6500);}}
