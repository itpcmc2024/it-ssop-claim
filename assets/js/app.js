...
/* ZIP Reader V3.2.0 — local browser processing */
const zipReaderState={zip:null,file:null,entries:[],selected:null,rows:[],headers:[],caseItem:null};
...
function resetZipReader(){zipReaderState.zip=null;zipReaderState.file=null;zipReaderState.entries=[];zipReaderState.selected=null;zipReaderState.rows=[];zipReaderState.headers=[];const i=document.getElementById('zipFileInput');if(i)i.value='';document.getElementById('zipChooseStep')?.classList.remove('hidden');document.getElementById('zipWorkspace')?.classList.add('hidden')}
function baseName(path){return String(path||'').split('/').filter(Boolean).pop()||path}
function classifySsopFile(name){const n=baseName(name).toUpperCase().replace(/\.[^.]+$/,'');if(n.includes('BILLTRAN'))return'BILLTRAN';if(n.includes('BILLDISP')||n.includes('BILLITEM'))return'BILLDISP / BillItems';if(n.includes('OPSERVICE'))return'OPServices';if(n.includes('OPDX'))return'OPDx';if(n.includes('DISPENS'))return'DispensItems';return'ไฟล์อื่น'}
...
async function handleZipFile(file){if(!file)return;if(!window.JSZip){toast('เปิด ZIP ไม่ได้','ไม่พบไลบรารี JSZip กรุณาตรวจสอบอินเทอร์เน็ต','error');return}if(!/\.zip$/i.test(file.name)){toast('ชนิดไฟล์ไม่ถูกต้อง','กรุณาเลือกไฟล์ .zip','warning');return}try{
 const summary=document.getElementById('zipSummary');document.getElementById('zipChooseStep').classList.add('hidden');document.getElementById('zipWorkspace').classList.remove('hidden');summary.innerHTML='<div class="zip-loading">กำลังอ่านโครงสร้าง ZIP...</div>';
 const zip=await JSZip.loadAsync(file);const entries=Object.values(zip.files).filter(e=>!e.dir).map(e=>({name:e.name,size:e._data?.uncompressedSize||0,type:classifySsopFile(e.name),entry:e}));
 zipReaderState.zip=zip;zipReaderState.file=file;zipReaderState.entries=entries;
 const expected=['BILLTRAN','OPServices','OPDx','BILLDISP / BillItems','DispensItems'];const present=new Set(entries.map(e=>e.type));const missing=expected.filter(x=>!present.has(x));
 summary.innerHTML=`<div class="summary-card"><span>ชื่อ ZIP</span><strong>${escapeHtml(file.name)}</strong></div><div class="summary-card"><span>จำนวนไฟล์</span><strong>${entries.length}</strong></div><div class="summary-card"><span>ขนาด</span><strong>${(file.size/1024).toFixed(1)} KB</strong></div><div class="summary-card ${missing.length?'warn':''}"><span>ไฟล์มาตรฐานที่ยังไม่พบ</span><strong>${missing.length?escapeHtml(missing.join(', ')):'ครบตามรายการตรวจเบื้องต้น'}</strong></div>`;
...
function renderZipFileList(){const wrap=document.getElementById('zipFileList');wrap.innerHTML=zipReaderState.entries.length?zipReaderState.entries.map(e=>`<button class="zip-file-item ${zipReaderState.selected===e.name?'active':''}" data-zip-entry="${escapeHtml(e.name)}"><span class="zip-file-icon">📄</span><span class="grow"><b>${escapeHtml(baseName(e.name))}</b><small>${escapeHtml(e.type)} · ${(e.size/1024).toFixed(1)} KB</small></span></button>`).join(''):'<div class="empty-row">ไม่พบไฟล์ภายใน ZIP</div>';wrap.querySelectorAll('[data-zip-entry]').forEach(b=>b.onclick=()=>selectZipEntry(b.dataset.zipEntry))}
async function selectZipEntry(name){const item=zipReaderState.entries.find(e=>e.name===name);if(!item)return;zipReaderState.selected=name;renderZipFileList();document.getElementById('zipPreviewTitle').textContent=baseName(name);document.getElementById('zipPreviewMeta').textContent=`${item.type} · กำลังอ่านข้อมูล...`;document.getElementById('zipPreviewBody').innerHTML='<tr><td class="empty-row">กำลังอ่านไฟล์...</td></tr>';try{const buf=await item.entry.async('arraybuffer');const text=decodeSsopBuffer(buf);const parsed=parseSsopText(text);zipReaderState.headers=parsed.headers;zipReaderState.rows=parsed.rows;document.getElementById('zipPreviewMeta').textContent=`${item.type} · ${parsed.rows.length.toLocaleString('th-TH')} แถว · ${parsed.headers.length} คอลัมน์`;const search=document.getElementById('zipTableSearch');search.disabled=false;search.value='';renderZipPreview()}catch(err){document.getElementById('zipPreviewBody').innerHTML=`<tr><td class="empty-row">อ่านไฟล์ไม่ได้: ${escapeHtml(err.message)}</td></tr>`}}
function renderZipPreview(){const q=(document.getElementById('zipTableSearch')?.value||'').trim().toLowerCase();const all=zipReaderState.rows;const filtered=q?all.filter(r=>r.some(v=>String(v||'').toLowerCase().includes(q))):all;const rows=filtered.slice(0,500);document.getElementById('zipPreviewHead').innerHTML=`<tr><th>#</th>${zipReaderState.headers.map(h=>`<th>${escapeHtml(h)}</th>`).join('')}</tr>`;document.getElementById('zipPreviewBody').innerHTML=rows.length?rows.map((r,i)=>`<tr><td>${i+1}</td>${zipReaderState.headers.map((_,c)=>`<td>${escapeHtml(r[c]??'')}</td>`).join('')}</tr>`).join(''):`<tr><td colspan="${zipReaderState.headers.length+1}" class="empty-row">ไม่พบข้อมูล</td></tr>`;const meta=document.getElementById('zipPreviewMeta');if(filtered.length>500)meta.textContent+=` · แสดง 500 จาก ${filtered.length.toLocaleString('th-TH')} แถว`}
document.getElementById('openZipReaderBtn')?.addEventListener('click',()=>openZipReader());document.getElementById('zipReaderClose')?.addEventListener('click',closeZipReader);document.getElementById('zipChooseBtn')?.addEventListener('click',()=>document.getElementById('zipFileInput').click());document.getElementById('zipFileInput')?.addEventListener('change',e=>handleZipFile(e.target.files?.[0]));document.getElementById('zipChangeBtn')?.addEventListener('click',resetZipReader);document.getElementById('zipTableSearch')?.addEventListener('input',renderZipPreview);const zipDrop=document.getElementById('zipDropZone');if(zipDrop){['dragenter','dragover'].forEach(ev=>zipDrop.addEventListener(ev,e=>{e.preventDefault();zipDrop.classList.add('dragover')}));['dragleave','drop'].forEach(ev=>zipDrop.addEventListener(ev,e=>{e.preventDefault();zipDrop.classList.remove('dragover')}));zipDrop.addEventListener('drop',e=>handleZipFile(e.dataTransfer.files?.[0]))}
