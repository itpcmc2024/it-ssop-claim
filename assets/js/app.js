/*
======================================================
SSOP Toolkit Professional Edition V3.2.1
...
const zipReaderState={zip:null,file:null,entries:[],selected:null,rows:[],headers:[],sections:{},activeSection:'',caseItem:null};
...
async function handleZipFile(file){if(!file)return;if(!window.JSZip){toast('เปิด ZIP ไม่ได้','ไม่พบไลบรารี JSZip กรุณาตรวจสอบอินเทอร์เน็ต','error');return}if(!/\.zip$/i.test(file.name)){toast('ชนิดไฟล์ไม่ถูกต้อง','กรุณาเลือกไฟล์ .zip','warning');return}try{
 ...
 for(const e of rawEntries){let logicalSections=[];try{const buf=await e.async('arraybuffer');const text=decodeSsopBuffer(buf);logicalSections=Object.keys(parseZipSsopSections(text));logicalSections.forEach(x=>logicalPresent.add(x))}catch(_e){}
  entries.push({name:e.name,size:e._data?.uncompressedSize||0,type:classifySsopFile(e.name),logicalSections,entry:e});
 }
...
function renderZipPreview(){const q=(document.getElementById('zipTableSearch')?.value||'').trim().toLowerCase();const all=zipReaderState.rows;const filtered=q?all.filter(r=>r.some(v=>String(v||'').toLowerCase().includes(q))):all;const rows=filtered.slice(0,500);const importantCols=(sectionInfo[zipReaderState.activeSection]?.importantCols)||[];document.getElementById('zipPreviewHead').innerHTML=`<tr><th>#</th>${zipReaderState.headers.map((h,i)=>`<th class="${importantCols.includes(i)?'important-head':''}">${escapeHtml(h)}${importantCols.includes(i)?' ★':''}</th>`).join('')}</tr>`;document.getElementById('zipPreviewBody').innerHTML=rows.length?rows.map((r,i)=>`<tr><td>${i+1}</td>${zipReaderState.headers.map((_,c)=>`<td class="${importantCols.includes(c)?'important-cell':''}" title="${escapeAttr(r[c]??'')}">${escapeHtml(r[c]??'')}</td>`).join('')}</tr>`).join(''):`<tr><td colspan="${zipReaderState.headers.length+1}" class="empty-row">ไม่พบข้อมูล</td></tr>`;if(filtered.length>500){const meta=document.getElementById('zipPreviewMeta');meta.textContent+=` · แสดง 500 จาก ${filtered.length.toLocaleString('th-TH')} แถว`}}
