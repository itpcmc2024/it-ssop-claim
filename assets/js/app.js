...
function renderCrossPreflight_(){
 ...
 const active=document.getElementById('crossIssueCodeFilter')?.value||selected;const bulkBtn=document.getElementById('crossBulkFixBtn');if(bulkBtn){bulkBtn.disabled=!CROSS_BULK_FIXABLE.has(active);bulkBtn.textContent=CROSS_BULK_FIXABLE.has(active)?`🛠️ แก้ ${active} ทั้งไฟล์`:'🛠️ Error นี้ยังต้องตรวจเอง';}
