/*
======================================================
PCMC-SSO Toolkit Professional Edition V4.5.5
...
const REGISTRY_MODULE_CONFIG={
 SSOCAC:{title:'ทะเบียนงาน Cancer Care',subtitle:'ทะเบียนผู้ป่วยหลัง Discharge สำหรับเตรียมข้อมูลส่งเบิก SSOCAC',icon:'🎗️',footer:'PCMC-SSO Toolkit · Cancer Care Registry Version 4.5.5'},
 STCPAP:{title:'ทะเบียนงาน SSOCPAP',subtitle:'ทะเบียนผู้ป่วยสำหรับเตรียมข้อมูลส่งเบิกเครื่อง CPAP และหน้ากาก ตามกฎ STCPAP',icon:'🫁',footer:'PCMC-SSO Toolkit · SSOCPAP Registry Version 4.5.5'},
 STSLEEP:{title:'ทะเบียนงาน Sleep Test',subtitle:'ทะเบียนผู้ป่วยสำหรับเตรียมข้อมูลส่งเบิก Sleep Test ตามกฎ STCPAP',icon:'🌙',footer:'PCMC-SSO Toolkit · Sleep Test Registry Version 4.5.5'}
};
...
function openZipReader(caseId='',explicitModule=''){
 ...
 const dropTitle=document.getElementById('zipDropTitle'),dropHint=document.getElementById('zipDropHint');if(dropTitle)dropTitle.textContent=moduleCode==='CROSS'?'ลาก ZIP ข้ามเขต (หลายผู้ป่วย) มาวางที่นี่':'ลาก ZIP ผู้ป่วยมาวางที่นี่';if(dropHint)dropHint.textContent=moduleCode==='CROSS'?'รองรับ ZIP SSOP ที่รวมผู้ป่วยหลายราย และตรวจความสัมพันธ์ข้าม BILLTRAN/BILLDISP/OPServices':'รองรับ 1 ZIP ต่อผู้ป่วย 1 ราย';
...
const CROSS_RULE_GUIDE={
 C07:{level:'Warning',title:'Hmain อาจไม่ถูกต้อง',solution:'ตรวจ BILLTRAN.Hmain ให้เป็นรหัสสถานพยาบาลหลักที่ถูกต้อง ณ BILLTRAN.DTTran; ระบบตรวจค่าว่าง/รูปแบบได้ แต่การยืนยันสิทธิจริงต้องเทียบฐานสิทธิผู้ประกันตนตามวันรักษา'},
 ...
 R04:{level:'Error',title:'ยอด Dispensing / DispensedItems ไม่ตรงกัน',solution:'ตรวจ DispID เดียวกัน แล้วเทียบ Dispensing.ChargeAmt กับผลรวม DispensedItems.ChargeAmt และ Dispensing.ClaimAmt กับผลรวม DispensedItems.ReimbAmt จากนั้นแก้ยอดหรือรายการยาที่ขาด/เกิน'},
 ...
 S14:{level:'Warning',title:'DTAppoint ไม่ถูกต้อง',solution:'ถ้ามีวันนัด ให้ใช้รูปแบบ YYYY-MM-DD และต้องไม่ก่อนวันให้บริการ BegDT; ถ้าไม่มีนัดให้คงค่าว่างตามข้อมูลต้นทาง'},
 S18:{level:'Warning',title:'DiagnosisCode / CodeSet น่าสงสัย',solution:'ตรวจ OPDx.CodeSet และ DiagnosisCode ให้เป็นชุดรหัสที่กองทุนยอมรับ; ระบบตรวจรูปแบบเบื้องต้นได้ แต่การยืนยันรหัสจริงต้องเทียบฐาน ICD/codeset อ้างอิง'},
 S19:{level:'Warning',title:'รหัสบริการ / CodeSet น่าสงสัย',solution:'ตรวจ OPServices.CodeSet และ STDCode/LCCode ให้สัมพันธ์กับประเภทบริการ; การยืนยันรหัสจริงต้องใช้ตาราง codeset อ้างอิง'},
 ...
 S41:{level:'Warning',title:'Class เป็นหัตถการแต่ขาดรหัสบริการ',solution:'เมื่อ OPServices.Class เป็น OP/หัตถการ ต้องระบุรหัสหัตถการใน LCCode หรือ STDCode และ CodeSet ให้ครบ'},
 ...
 T06:{level:'Warning',title:'สิทธิประกันสุขภาพหลักต้องตรวจฐานอ้างอิง',solution:'ตรวจ PayPlan/Hmain กับสิทธิผู้ป่วยและรหัสกองทุนล่าสุด; ระบบเตือนค่าว่าง/รูปแบบผิดได้ แต่ต้องมี master สิทธิสำหรับยืนยัน T06 แบบเต็ม'},
 ...
 T42:{level:'Warning',title:'SvDate ไม่สัมพันธ์กับ DTTran',solution:'ตรวจ BillItems.SvDate กับ BILLTRAN.DTTran ของ InvNo เดียวกัน โดยทั่วไปควรเป็นวันบริการเดียวกัน; ถ้าต่างวันให้ตรวจ visit/การลงวันที่ต้นทาง'},
 ...
 W04:{level:'Reference',title:'ช่วงวันราคายาต้องตรวจ Drugcatalog',solution:'ต้องมี Drugcatalog ที่ระบุ Effective/Expire date แล้วเทียบ BILLTRAN.DTTran/DispDate กับช่วงวันที่ราคายามีผล'},
 W05:{level:'Reference',title:'ราคายาต้องตรวจ Drugcatalog',solution:'ต้องเชื่อม HospDrgID/DrgID กับ Drugcatalog แล้วเทียบ UnitPrice/ReimbPrice กับราคาที่ประกาศใช้ในวันบริการ'},
 W07:{level:'Reference',title:'TMT ต้องตรง Drugcatalog',solution:'ต้องเชื่อม DispensedItems.HospDrgID กับ Drugcatalog แล้วตรวจ DrgID (TMT) ให้ตรงกัน หากไม่ตรงให้แก้ master drug หรือแฟ้มส่งตามข้อมูลจริง'}
};
...
function scanCrossCustomRules_(sec,issues){const rules=loadCrossCustomRules_().filter(r=>r&&r.enabled!==false&&r.code&&r.section&&r.field);for(const rule of rules){const rows=sec[rule.section]||[];const hdr=sectionHeaders(rule.section,rows.map(x=>x.row));const col=hdr.indexOf(rule.field);if(col<0)continue;rows.forEach(o=>{if(crossCustomRuleMatch_(o.row[col],rule))crossAddIssue_(issues,String(rule.code).toUpperCase(),o,`${rule.field} = ${String(o.row[col]??'(ว่าง)')}`,String(o.row[col]??''),{level:rule.level||'Warning',title:rule.title||rule.code,solution:rule.solution||'ตรวจและแก้ค่าฟิลด์นี้ตามข้อมูลจริง',targetCol:col,custom:true})})}}
...
async function bulkFixSelectedCrossCode_(){const code=document.getElementById('crossIssueCodeFilter')?.value||'ALL';if(!CROSS_BULK_FIXABLE.has(code)){toast('ยังไม่รองรับการแก้อัตโนมัติทั้งหมด','เลือก Error Code ที่ระบบคำนวณค่าแก้ไขได้อย่างปลอดภัย เช่น C08, S32, S33, R04, T33, T44 หรือ T45','warning',6000);return;}const issues=(zipReaderState.crossIssues||[]).filter(x=>x.code===code&&x.entry);if(!issues.length)return;const ok=await showDialog('แก้รายการทั้งหมดของ '+code,`พบ ${issues.length} จุด ระบบจะใช้ค่าที่คำนวณจาก key เดียวกันแก้ทุกจุดของ Error Code ${code}\\n\\nก่อนบันทึกจริงยังสามารถตรวจและ Undo ไฟล์แต่ละไฟล์ได้`,'warning',[{text:'ยกเลิก',value:false,className:'soft'},{text:'แก้ทั้งหมด',value:true,className:'primary'}]);if(!ok)return;const ix=crossSectionIndex_();let changed=0;
...
function renderCrossValueReplace_(){
 ...
 document.getElementById('crossValueReplaceTitle').textContent=`แทนค่าซ้ำใน ${code} · ${fieldNames.join(' / ')}`;
 document.getElementById('crossValueReplaceMeta').textContent=`ระบบจะแก้เฉพาะ ${issues.length} จุดที่ถูกตรวจเป็น ${code} เท่านั้น ไม่กระทบแถวอื่น`;
...
function renderCrossPreflight_(){
 ...
 const active=document.getElementById('crossIssueCodeFilter')?.value||selected;const bulkBtn=document.getElementById('crossBulkFixBtn');if(bulkBtn){bulkBtn.disabled=!CROSS_BULK_FIXABLE.has(active);bulkBtn.textContent=CROSS_BULK_FIXABLE.has(active)?`✨ คำนวณแก้ ${active} ทั้งหมด`:'✨ ไม่มี Auto Fix แบบคำนวณ';}
 ...
 document.getElementById('crossIssueBody').innerHTML=filtered.length?filtered.slice(0,500).map(x=>{const t=crossTargetForIssue_(x);return `<tr><td><b>${x.code}</b>${x.custom?'<br><small>Custom</small>':''}</td><td><span class="cross-level ${x.level.toLowerCase()}">${x.level}</span></td><td>${escapeHtml(x.section||'-')}${x.rowIndex>=0?` แถว ${x.rowIndex+1}`:''}<br><small>${escapeHtml(x.entry?baseName(x.entry):'ฐานอ้างอิง')}</small></td><td>${t?`<b>${escapeHtml(t.field)}</b>`:'<span class="meta">หลายฟิลด์/ต้องตรวจบริบท</span>'}</td><td>${escapeHtml(x.detail)}</td><td>${escapeHtml(x.solution)}</td><td>${x.entry?`${zipRowWasEdited_(x.entry,x.section,x.rowIndex)?'<span class="cross-edited-badge">✓ แก้แล้ว</span> ':''}<button class="primary mini" data-cross-jump="${issues.indexOf(x)}">${t?'แก้ฟิลด์นี้':'ดูจุดนี้'}</button>`:'<span class="meta">ต้องเชื่อมฐานอ้างอิง</span>'}</td></tr>`}).join(''):'<tr><td colspan="7" class="empty-row">ไม่พบรายการตามตัวกรอง</td></tr>';
...
function openCrossRuleManager_(){document.getElementById('crossRuleManagerModal')?.classList.add('show');document.getElementById('crossRuleManagerModal')?.setAttribute('aria-hidden','false');syncCrossRuleFields_();renderCrossRuleManager_();}
...
function renderCrossRuleManager_(){const body=document.getElementById('crossRuleBody');if(!body)return;const rules=loadCrossCustomRules_();body.innerHTML=rules.length?rules.map((r,i)=>`<tr><td><input type="checkbox" data-cross-rule-toggle="${i}" ${r.enabled!==false?'checked':''}></td><td><b>${escapeHtml(r.code)}</b></td><td>${escapeHtml(r.section)}.${escapeHtml(r.field)}</td><td>${escapeHtml(r.operator)} ${escapeHtml(r.value||'')}</td><td>${escapeHtml(r.title||'')}</td><td><button class="danger mini" data-cross-rule-delete="${i}">ลบ</button></td></tr>`).join(''):'<tr><td colspan="6" class="empty-row">ยังไม่มีกฎ Custom</td></tr>';body.querySelectorAll('[data-cross-rule-toggle]').forEach(x=>x.onchange=()=>{const a=loadCrossCustomRules_();a[+x.dataset.crossRuleToggle].enabled=x.checked;saveCrossCustomRules_(a);runCrossPreflight()});body.querySelectorAll('[data-cross-rule-delete]').forEach(x=>x.onclick=async()=>{const a=loadCrossCustomRules_(),r=a[+x.dataset.crossRuleDelete];const ok=await showDialog('ลบกฎ Custom',`ลบ ${r?.code||''} ${r?.title||''} ?`,'warning',[{text:'ยกเลิก',value:false,className:'soft'},{text:'ลบ',value:true,className:'danger'}]);if(!ok)return;a.splice(+x.dataset.crossRuleDelete,1);saveCrossCustomRules_(a);renderCrossRuleManager_();runCrossPreflight()});}
function addCrossCustomRule_(){const code=String(document.getElementById('crossRuleCode')?.value||'').trim().toUpperCase(),section=document.getElementById('crossRuleSection')?.value,field=document.getElementById('crossRuleField')?.value,operator=document.getElementById('crossRuleOperator')?.value,value=document.getElementById('crossRuleValue')?.value||'',level=document.getElementById('crossRuleLevel')?.value||'Warning',title=String(document.getElementById('crossRuleTitle')?.value||'').trim(),solution=String(document.getElementById('crossRuleSolution')?.value||'').trim();if(!/^[A-Z][A-Z0-9]{1,5}$/.test(code)||!section||!field||!title){toast('ข้อมูลกฎยังไม่ครบ','กรอก Error Code, Section, Field และชื่อกฎ','warning');return;}const a=loadCrossCustomRules_();a.push({id:Date.now(),enabled:true,code,section,field,operator,value,level,title,solution});saveCrossCustomRules_(a);document.getElementById('crossRuleCode').value='';document.getElementById('crossRuleTitle').value='';document.getElementById('crossRuleSolution').value='';renderCrossRuleManager_();runCrossPreflight();toast('เพิ่มกฎ Cross แล้ว',`${code} จะถูกตรวจทันทีใน Browser นี้`,'success');}
...
function runCrossPreflight(){
 ...
 zipReaderState.crossIssues=scanCrossZip_();renderCrossPreflight_();const n=zipReaderState.crossIssues.length;updateZipEditStatus(n?`Cross: พบ ${n} จุด`:'Cross: ตรวจผ่านเบื้องต้น');return zipReaderState.crossIssues;
}
