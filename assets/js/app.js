/*
======================================================
SSOP Toolkit Professional Edition V4.2.7
...
const dictionaryModal=document.getElementById('dictionaryModal');const openDictionary=()=>{const cpap=currentRegistryModule==='STCPAP';const sleep=currentRegistryModule==='STSLEEP';const title=dictionaryModal.querySelector('.modal-head strong');const meta=dictionaryModal.querySelector('.modal-head .meta');if(title)title.textContent=cpap?'📖 คู่มือฟิลด์ SSOCPAP':sleep?'📖 คู่มือฟิลด์ Sleep Test':'📖 คู่มือฟิลด์ SSOCAC';if(meta)meta.textContent=cpap?'คำอธิบายฟิลด์ เงื่อนไขสำคัญ และตัวอย่างจริงสำหรับการเบิกเครื่อง CPAP/หน้ากาก':sleep?'คำอธิบายฟิลด์สำคัญสำหรับการเบิก Sleep Test ชนิดที่ 1 และ 2':'ใช้สำหรับเปิดดูความหมาย เงื่อนไข และตัวอย่างของแต่ละฟิลด์';dictionaryModal.classList.add('show');dictionaryModal.setAttribute('aria-hidden','false');renderDictionary();};document.getElementById('dictionaryBtn').onclick=openDictionary;
const replyModuleSelect=document.getElementById('replyModule');
function syncEditorModuleUi(){const v=String(replyModuleSelect?.value||'SSOCAC').toUpperCase();const btn=document.getElementById('dictionaryBtn');if(btn){btn.dataset.guideModule=v;const labels={SSOCAC:'SSOCAC',STCPAP:'CPAP',STSLEEP:'Sleep Test',MAIN:'Main',CROSS:'ข้ามเขต'};btn.textContent='📖 คู่มือฟิลด์ '+(labels[v]||'SSOP');}}
...
function parseSSOCACReplyKnowledge(text){
  ...
  return [...counts.entries()].map(([code,count])=>({
    module:replyKnowledgeState.module||'SSOCAC',code,count,
...
function exportReplyKnowledgeCSV(){
  ...
  a.href=URL.createObjectURL(blob);a.download=`${replyKnowledgeState.module||'SSOP'}_Error_Knowledge_${new Date().toISOString().slice(0,10)}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
...
async function hydrateReplyKnowledgeFromDatabase(){
  try{
    const data=await apiRequest('getByCodes',{module:replyKnowledgeState.module||'SSOCAC',codes:replyKnowledgeState.items.map(x=>x.code)});
...
async function saveReplyKnowledge(){
  ...
  try{
    const items=sourceItems.map(x=>({
      Module:replyKnowledgeState.module||'SSOCAC',ErrorCode:x.code,Description:x.description,Cause:x.cause,Solution:x.solution,
...
function knowledgeCard(item,index){
 ...
 return `<details class="knowledge-compact-row ${complete?'complete':'incomplete'}" data-knowledge-index="${index}">
   <summary><span class="knowledge-code-badge">${escapeHtml(item.ErrorCode||'-')}</span><span class="knowledge-module-mini">${escapeHtml(item.Module||'-')}</span><strong class="knowledge-description">${escapeHtml(item.Description||'ยังไม่มีคำอธิบาย')}</strong><span class="knowledge-status-mini">${complete?'✓ มีวิธีแก้':'รอเติมวิธีแก้'}</span><span class="knowledge-chevron">⌄</span></summary>
...
async function hydrateRegistryErrorKnowledge(){
 ...
 try{const data=await apiRequest('getByCodes',{module:'SSOCAC',codes});(data.items||[]).forEach(k=>registryState.errorKnowledge.set(String(k.ErrorCode||'').toUpperCase(),k));}
...
function exportErrorQueueCsv(){
 ...
 const csv='\uFEFF'+[headers,...rows].map(r=>r.map(csvCell).join(',')).join('\r\n');
 const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}),a=document.createElement('a');
 a.href=URL.createObjectURL(blob);a.download=`SSOCAC_${code}_Work_Queue_${new Date().toISOString().slice(0,10)}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
 toast('ส่งออก CSV แล้ว',`${code} จำนวน ${rows.length} รายการ`,'success');
}


const REGISTRY_MODULE_CONFIG={
 SSOCAC:{title:'ทะเบียนงาน Cancer Care',subtitle:'ทะเบียนผู้ป่วยหลัง Discharge สำหรับเตรียมข้อมูลส่งเบิก SSOCAC',icon:'🎗️',footer:'SSOP Toolkit · Cancer Care Registry Version 4.2.7'},
 STCPAP:{title:'ทะเบียนงาน SSOCPAP',subtitle:'ทะเบียนผู้ป่วยสำหรับเตรียมข้อมูลส่งเบิกเครื่อง CPAP และหน้ากาก ตามกฎ STCPAP',icon:'🫁',footer:'SSOP Toolkit · SSOCPAP Registry Version 4.2.7'},
 STSLEEP:{title:'ทะเบียนงาน Sleep Test',subtitle:'ทะเบียนผู้ป่วยสำหรับเตรียมข้อมูลส่งเบิก Sleep Test ตามกฎ STCPAP',icon:'🌙',footer:'SSOP Toolkit · Sleep Test Registry Version 4.2.7'}
};
...
async function loadRegistry(){
 ...
 try{
  const data=await apiRequest('listCases',{module:currentRegistryModule,limit:5000});
  registryState.items=data.items||[];
  applyRegistryFilter();
  status.textContent=`เชื่อมต่อฐานข้อมูลแล้ว · ${registryState.items.length} รายการ`;
  await hydrateRegistryErrorKnowledge();
 }catch(err){
  status.textContent=err.message;
  document.getElementById('registryBody').innerHTML=`<tr><td colspan="12" class="empty-row">${escapeHtml(err.message)}</td></tr>`;
 }
}
...
function exportRegistryCsv(){
 ...
 const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}),a=document.createElement('a');
 a.href=URL.createObjectURL(blob);a.download=`${currentRegistryModule}_Registry_Filtered_${new Date().toISOString().slice(0,10)}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
...
const EXCEL_HEADERS_SLEEP=['ปชช','VN','HN','ชื่อผู้ป่วย','วันที่รับบริการ','เลขกำกับเบิก','tflag','session','station','JobNo'];
function openExcelImport(){resetExcelImport();const cpap=currentRegistryModule==='STCPAP';const sleep=currentRegistryModule==='STSLEEP';const m=document.getElementById('excelImportModal');const title=m?.querySelector('.modal-head strong');const meta=m?.querySelector('.modal-head .meta');if(title)title.textContent=cpap?'📥 นำเข้า Excel — SSOCPAP':'📥 นำเข้า Excel — Cancer Care';if(meta)meta.textContent=cpap?'รองรับชีต CPAP และตรวจรายการซ้ำก่อนบันทึก':'รองรับรูปแบบไฟล์ตัวอย่าง DATA และตรวจรายการซ้ำก่อนบันทึก';m.classList.add('show');m.setAttribute('aria-hidden','false')}
...
function openReplyImport(){resetReplyImport();const cpap=currentRegistryModule==='STCPAP';const sleep=currentRegistryModule==='STSLEEP';const m=document.getElementById('replyImportModal');const title=m?.querySelector('.modal-head strong');const meta=m?.querySelector('.modal-head .meta');if(title)title.textContent=cpap?'📨 นำเข้าผลตอบกลับ SSOCPAP':'📨 นำเข้าผลตอบกลับ Cancer Care';if(meta)meta.textContent=cpap?'อ่าน ZIP/BIL ตอบกลับ จับคู่ด้วยงวดส่ง Session : Station และบันทึก Error Code':'อ่านไฟล์ SOCDBIL และเชื่อมตามงวดส่ง';m.classList.add('show');m.setAttribute('aria-hidden','false')}
