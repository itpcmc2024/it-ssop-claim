/*
======================================================
SSOP Toolkit Professional Edition V3.8.2
...
const zipReaderState={zip:null,file:null,entries:[],selected:null,rows:[],headers:[],sections:{},activeSection:'',caseItem:null,dirty:false,rowSelections:{}};
function openZipReader(caseId=''){
 ...
 label.textContent=zipReaderState.caseItem?`${zipReaderState.caseItem.Case_ID} · HN ${zipReaderState.caseItem.HN||'-'} · ${zipReaderState.caseItem.Patient_Name||''} · Session ${zipReaderState.caseItem.Session||'-'} : Station ${zipReaderState.caseItem.Station||'-'}`:'อ่านไฟล์ ZIP ภายใน Browser โดยไม่อัปโหลดไฟล์ผู้ป่วยขึ้นฐานข้อมูล';
...
function updateZipRowTools(){
 ...
 if(del)del.disabled=selected.size===0;if(add)add.disabled=!zipReaderState.selected||!zipReaderState.activeSection;if(count)count.textContent=`เลือก ${selected.size.toLocaleString('th-TH')} แถว`;
...
function markCurrentZipModified(){const item=currentZipItem();if(item){item.modified=true;item.sections=zipReaderState.sections}zipReaderState.dirty=true;updateZipEditStatus()}
function addZipRow(){
 if(!zipReaderState.selected||!zipReaderState.activeSection){toast('ยังเพิ่มแถวไม่ได้','กรุณาเลือกไฟล์และส่วนข้อมูลก่อน','warning');return}
 const rows=zipReaderState.sections[zipReaderState.activeSection]||[];const width=Math.max(zipReaderState.headers.length,(fieldMeta[zipReaderState.activeSection]||[]).length,1);rows.push(Array(width).fill(''));zipReaderState.rows=rows;zipReaderState.headers=sectionHeaders(zipReaderState.activeSection,rows);markCurrentZipModified();renderZipSectionTabs();renderZipPreview();
 const tableWrap=document.querySelector('.zip-table-wrap');if(tableWrap)setTimeout(()=>{tableWrap.scrollTop=tableWrap.scrollHeight},0);toast('เพิ่มแถวแล้ว',`เพิ่มแถวว่างใน ${zipReaderState.activeSection} กรุณากรอกข้อมูลให้ครบ`,'success',3500)
}
async function deleteSelectedZipRows(){
 const selected=currentZipSelection();if(!selected.size)return;const count=selected.size;
 const ok=await confirmDialog('ยืนยันการลบแถว',`ต้องการลบ ${count.toLocaleString('th-TH')} แถวที่เลือกจาก ${zipReaderState.activeSection} ใช่หรือไม่?\n\nสามารถกด “คืนค่าไฟล์นี้” เพื่อย้อนกลับเป็นไฟล์ต้นฉบับได้`,'ลบแถว');if(!ok)return;
 const rows=zipReaderState.sections[zipReaderState.activeSection]||[];[...selected].sort((a,b)=>b-a).forEach(i=>{if(i>=0&&i<rows.length)rows.splice(i,1)});selected.clear();zipReaderState.rows=rows;zipReaderState.headers=sectionHeaders(zipReaderState.activeSection,rows);markCurrentZipModified();renderZipSectionTabs();renderZipPreview();toast('ลบแถวแล้ว',`ลบ ${count.toLocaleString('th-TH')} แถวจาก ${zipReaderState.activeSection}`,'success',3500)
}
...
async function undoZipEntry(){const item=currentZipItem();if(!item)return;item.text=item.originalText;item.sections=parseZipSsopSections(item.originalText||'');item.modified=false;item.autoChangedCells={};Object.keys(zipReaderState.rowSelections).filter(k=>k.startsWith(`${item.name}::`)).forEach(k=>delete zipReaderState.rowSelections[k]);zipReaderState.sections=item.sections;zipReaderState.dirty=false;const names=Object.keys(item.sections);if(names.length)selectZipSection(names[0]);updateZipEditStatus('คืนค่าไฟล์เดิมแล้ว')}
