(function(){'use strict';
 ...
 async function buildZip(state,selectedPaths){const zip=new JSZip();const selected=selectedPaths instanceof Set?selectedPaths:null;for(const e of state.entries){if(e.isXml){if(selected&&!selected.has(e.path))continue;const finalText=withEndNote(e.text);zip.file(e.path,encode874(finalText));}else zip.file(e.path,e.bytes)}return zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}})}
